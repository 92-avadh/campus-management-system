...
const Dashboard = () => {
  ...
  // Handlers (Approve/Reject/Add User/Logout)...
  const handleApprove = async (id) => {
    if (!window.confirm("Approve student?")) return;
    setLoading(true);
    try {
      const res = await fetch(`http://localhost:5000/api/admin/approve/${id}`, { method: "POST" });
      if (res.ok) { alert("✅ Approved!"); fetchApplications(); }
  ...
  return (
    // MAIN CONTAINER: Added dark:bg-gray-900 dark:text-white
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 font-sans transition-colors duration-300">
      ...
      <header className="bg-white dark:bg-gray-800 shadow p-4 flex justify-between items-center border-t-4 border-red-700 sticky top-0 z-50 transition-colors">
        ...
        <div className="flex items-center gap-6">
          ...
          <button onClick={handleLogout} className="text-sm font-bold text-gray-500 dark:text-gray-300 hover:text-red-600">
            LOGOUT
          </button>
          ...
          <div className="relative">
            <button 
              ...
              className={`w-10 h-10 rounded-full bg-red-700 text-white text-2xl flex items-center justify-center shadow-lg transition-transform duration-300 ${showMenu ? "rotate-45" : "rotate-0"} hover:bg-red-800`}
            >
              +
            </button>
            ...
            {showMenu && (
              <div className="absolute right-0 mt-3 w-48 bg-white dark:bg-gray-800 rounded-xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden z-50">
                <button onClick={() => openModal("faculty")} className="w-full text-left px-6 py-3 hover:bg-red-50 dark:hover:bg-gray-700 text-gray-700 dark:text-white font-bold border-b border-gray-100 dark:border-gray-700">
                  👨‍🏫 Add Faculty
                </button>
                <button onClick={() => openModal("admin")} className="w-full text-left px-6 py-3 hover:bg-red-50 dark:hover:bg-gray-700 text-gray-700 dark:text-white font-bold">
                  🛡️ Add Admin
                </button>
              </div>
            )}
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                {applications.map((app) => (
                  <tr key={app._id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                    ...
                    <td className="p-4 text-sm text-gray-500 dark:text-gray-400">{app.email}</td>
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center z-[100] backdrop-blur-sm">
          <div className="bg-white dark:bg-gray-800 w-full max-w-lg p-8 rounded-2xl shadow-2xl relative">
            ...
            <form onSubmit={handleAddUser} className="space-y-4">
              <input required placeholder="Full Name" className="w-full p-3 bg-gray-50 dark:bg-gray-700 dark:text-white border dark:border-gray-600 rounded outline-none" 
                value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
              
              <div className="flex gap-4">
                <input required placeholder="Email" type="email" className="w-1/2 p-3 bg-gray-50 dark:bg-gray-700 dark:text-white border dark:border-gray-600 rounded outline-none" 
                  value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} />
                <input required placeholder="Phone" className="w-1/2 p-3 bg-gray-50 dark:bg-gray-700 dark:text-white border dark:border-gray-600 rounded outline-none" 
                  value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
              </div>

              <div className="flex gap-4">
                <input required placeholder="User ID" className="w-1/2 p-3 bg-gray-50 dark:bg-gray-700 dark:text-white border dark:border-gray-600 rounded outline-none uppercase font-mono" 
                  value={formData.userId} onChange={e => setFormData({...formData, userId: e.target.value})} />
                <input required placeholder="Password" type="password" className="w-1/2 p-3 bg-gray-50 dark:bg-gray-700 dark:text-white border dark:border-gray-600 rounded outline-none" 
                  value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} />
              </div>

              <input required placeholder="Department" className="w-full p-3 bg-gray-50 dark:bg-gray-700 dark:text-white border dark:border-gray-600 rounded outline-none" 
