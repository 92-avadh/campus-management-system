...
const StudentTimetable = ({ user }) => {
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 animate-in fade-in">
      ...
      {/* ✅ MOBILE UI FIX: overflow-x-auto allows horizontal scrolling */}
      <div className="bg-white dark:bg-gray-800 rounded-[2rem] shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden overflow-x-auto">
        {schedule.length === 0 ? (
            <div className="p-10 text-center text-gray-400 font-bold">
                No upcoming classes found.
            </div>
        ) : (
            // ✅ Added min-w-[600px] to prevent squishing on mobile
                        ...
                        return (
                            <tr key={index} className={`transition-colors border-b last:border-0 ${slot.isCancelled ? "bg-red-50 dark:bg-red-900/10" : "hover:bg-gray-50 dark:hover:bg-gray-700/50"} ${live ? "bg-indigo-50/50 dark:bg-indigo-900/20" : ""}`}>
                                ...
                                <td className="p-6 font-bold text-gray-900 dark:text-white whitespace-nowrap align-top">
                                    <div className={slot.isCancelled ? "opacity-50" : ""}>{dateStr}</div>
                                </td>
                                ...
                                <td className="p-6 align-top">
                                    ...
                                    <div className="mt-1">
                                      {live && <span className="inline-block text-[10px] bg-indigo-500 text-white px-2 py-0.5 rounded-full animate-pulse font-bold tracking-wide">LIVE NOW</span>}
                                      {slot.isCancelled && <span className="inline-flex items-center gap-1 text-[10px] bg-red-100 text-red-600 px-2 py-0.5 rounded-full font-black uppercase tracking-wide border border-red-200"><FaTimesCircle size={10} /> Cancelled</span>}
                                    </div>
                                </td>

                                <td className={`p-6 font-bold align-top ${slot.isCancelled ? "text-gray-400 line-through" : "text-gray-900 dark:text-white"}`}>{slot.subject}</td>
                                <td className={`p-6 text-sm align-top ${slot.isCancelled ? "text-gray-300 line-through" : "text-gray-500 dark:text-gray-400"}`}>{slot.facultyName}</td>
                                <td className={`p-6 text-sm align-top ${slot.isCancelled ? "text-gray-300 line-through" : "text-gray-500 dark:text-gray-400"}`}>{slot.room}</td>
                            </tr>
                        );
                    })}
                </tbody>
